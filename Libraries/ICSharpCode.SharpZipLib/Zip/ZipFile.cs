            ...
            lock (baseStream_)
            {
                ...
                if (signature != ZipConstants.LocalHeaderSignature)
                {
                    throw new ZipException(
                        $"Wrong local header signature at 0x{entryAbsOffset:x}, expected 0x{ZipConstants.LocalHeaderSignature:x8}, actual 0x{signature:x8}");
                }
                ...
                if (testHeader)
                {
                    ...
                    {
                        throw new ZipException(
                            $"Central header/local header compression method mismatch ({entry.CompressionMethodForHeader:G} vs {compressionMethod:G})");
                    }
        ...
        */
        ...
        private void RunUpdates()
        {
            ...
            try
            {
                ...
                byte[] theComment = newComment_?.RawComment ??
                                    _stringCodec.ZipArchiveCommentEncoding.GetBytes(comment_);
        ...
        {
            return ZipFormat.LocateBlockWithSignature(baseStream_, signature, endLocation, minimumBlockSize,
                maximumVariableData);
        }
        ...
        private Stream CreateAndInitDecryptionStream(Stream baseStream, ZipEntry entry)
        {
            ...
            else
            {
                    ...
                    !entry.HasFlag(GeneralBitFlags.StrongEncryption))
                {
                    ...
                    result = new CryptoStream(baseStream, classicManaged.CreateDecryptor(key, null),
        ...
        public struct ZipEntryEnumerator : IEnumerator<ZipEntry>
        {
            ...
            /// <inheritdoc />
            public ZipEntry Current => array[index];
